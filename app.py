#!/usr/bin/env python3
"""
FastAPI backend for the Seismic Interactive dashboard.

This version is designed to work in two modes:
- Local/full mode: uses the bundled PyTorch ST-GNN model if torch is available.
- Lightweight/deployment mode: falls back to a pure-NumPy heuristic when torch is unavailable,
  which is better suited for small hosting plans such as Render's free tier.
"""
...
# ==========================================
# 2. Optional PyTorch ST-GNN Architecture
# ==========================================
if torch is not None and nn is not None:
    ...
    class EarthquakeSTGNN:  # pragma: no cover - fallback placeholder
        pass
...
async def lifespan(app: FastAPI):
    data = np.load(FEATURES_NPZ, allow_pickle=False, mmap_mode="r")
    model = None
    model_mode = "heuristic"

    if torch is not None and nn is not None and MODEL_PATH.exists():
        try:
    ...
    if "adj_matrix" in data:
        adj_matrix = np.asarray(data["adj_matrix"], dtype=np.float32)
    elif "adj_row" in data and "adj_col" in data and "adj_data" in data:
        num_nodes = len(np.asarray(data["coords"], dtype=np.float32))
        adj_matrix = np.zeros((num_nodes, num_nodes), dtype=np.float32)
        row_idx = np.asarray(data["adj_row"], dtype=np.int64)
        col_idx = np.asarray(data["adj_col"], dtype=np.int64)
        values = np.asarray(data["adj_data"], dtype=np.float32)
        adj_matrix[row_idx, col_idx] = values
        adj_matrix[col_idx, row_idx] = values
    else:
        adj_matrix = None
    ...
    state.update(
        {
            ...
            "adj_tensor": None if adj_matrix is None else torch.tensor(adj_matrix, dtype=torch.float32),
            "coords": np.asarray(data["coords"], dtype=np.float32),
            "fault_decay": np.asarray(data["fault_decay"], dtype=np.float32),
            "norm_dens": np.asarray(data["norm_dens"], dtype=np.float32),
            "vs30_amp": np.asarray(data["vs30_amp"], dtype=np.float32),
        }
    )
    yield
    state.clear()
...
def _build_heuristic_risk(req, coords, fault_decay, norm_dens, vs30_amp):
    dists = haversine_km(coords[:, 1], coords[:, 0], req.epicenter_y, req.epicenter_x)
    radius_km = float(10 ** (0.45 * req.magnitude - 1.1))
    proximity = np.clip(1.0 - (dists / max(radius_km, 1.0)), 0.0, 1.0)
    base = 0.18 + (req.magnitude - 4.0) / 25.0
    risk = base * 0.55 + proximity * 0.45 + 0.05 * np.asarray(vs30_amp, dtype=np.float32)
    return np.clip(risk, 0.0, 0.999)
...
@app.get("/health")
def health_check():
    return {"status": "ok", "mode": state.get("model_mode", "heuristic")}
...
def predict_risk(req: ScenarioRequest):
    ...
    if state.get("model") is not None and torch is not None:
        ...
        with torch.no_grad():
            raw_preds = state["model"](torch.tensor(scenario_tensor), torch.tensor(np.asarray(np.ones((num_nodes, num_nodes), dtype=np.float32))))
            raw_preds = raw_preds.squeeze(0).numpy()
        pga_hazard_index = np.clip(pga_g / 0.40, 0.0, 1.0)
        calibrated_risk = np.clip((pga_hazard_index * 0.75) + (raw_preds * 0.25), 0.0, 0.999)
    else:
        calibrated_risk = _build_heuristic_risk(req, coords, state["fault_decay"], state["norm_dens"], state["vs30_amp"])
    ...
    results = [
        {"coords": coords[i].tolist(), "predicted_risk": round(float(calibrated_risk[i]), 4)}
